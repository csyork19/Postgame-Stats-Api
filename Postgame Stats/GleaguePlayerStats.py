...
def get_gleague_player_playoff_stats(self, year):
    if self:
        player_logs = playergamelog.PlayerGameLog(
            player_id=PostGameStatsUtil.PostGameStatsUtil.get_gleague_player_id(self),
            season=year,
...
def get_gleague_player_shot_chart_coordinates(self, year):
    if self:
